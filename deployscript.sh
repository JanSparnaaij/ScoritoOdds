...
# ✅ Check if Playwright browsers are already installed to prevent reinstalling every time
if [ ! -d "/ms-playwright-browsers/chromium-1148" ]; then
    echo "📥 Installing Playwright browsers..."
    PLAYWRIGHT_BROWSERS_PATH=/ms-playwright-browsers playwright install --with-deps
else
    echo "✅ Playwright browsers already installed. Skipping reinstallation."
fi
